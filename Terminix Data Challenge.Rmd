...
m.data_1h_dup_Ref1 <- m.data_1h_dup$Ref1
m.dupe <- m.data_1h[is.element(m.data_1h$Ref1, m.data_1h_dup_Ref1),]
...
m.data_1h_no_dup_Ref1 <- m.data_1h_no_dup_Ref1 %>%
     remove_rownames() %>%
...
```{r}
m.smp_size <- floor(0.75 * nrow(m.data_1h_no_dup_Ref1))

# set the seed to make the partition reproducible
set.seed(123)
m.train_ind <- sample(seq_len(nrow(m.data_1h_no_dup_Ref1)), size = m.smp_size)

m.train <- m.data_1h_no_dup_Ref1[m.train_ind, ]
m.test <- m.data_1h_no_dup_Ref1[-m.train_ind, ]
...
# 
# summarizeColumns(m.train_cc) %>%
#   select(name,type,na,mean,median,min,max)%>%
#   mutate(percent_miss=(na/nrow(m.train_cc))*100)
## No missing values, but some variables are all 0 at this complete case scenario

# Remove columns that are all 0s
# m.train_cc_no0 <- m.train_cc[, colSums(m.train_cc != 0) > 0]
```
...
library(Boruta)
boruta_output <- Boruta(sale ~ ., data=na.omit(m.train), doTrace=0)  
...
set.seed(123)
m.train_ind <- sample(seq_len(nrow(m.data_1h_no_dup_Ref1)), size = m.smp_size)
m.data_sub <- m.data_1h_no_dup_Ref1 %>% select(sale, all_of(boruta_vars))
m.data_sub_noTarget <- m.data_1h_no_dup_Ref1 %>% select(all_of(boruta_vars))
# boruta_vars = c("CUSTOMER_CANCEL_REASON_OTHER", "BASE_PRICE", "BCC5520_AVG", "ILN6200A_PCT", "REV7430_AVG", "OPTIONS_PRICE", "BCC5421_AVG", "ALL6200A_PCT", "ALL6280A_PCT", "ILN7430_AVG")

train <- m.data_sub[m.train_ind, ]
test <- m.data_sub[-m.train_ind, ]

# Save them to csv
write.csv(x=train, file="./data/clean/train_sub.csv")
write.csv(x=test, file="./data/clean/test_sub.csv")
...
```{r}
summarizeColumns(m.data_sub_noTarget) %>%
  select(name,type,na,mean,median,min,max)%>%
  mutate(percent_miss=(na/nrow(m.data_sub_noTarget))*100)

# Create missing indicators, but some of them will be all 0
for (var in names(m.data_sub_noTarget)) {
    m.data_sub_noTarget[[paste0(var, "_missing")]] <- ifelse(is.na(m.data_sub_noTarget[[var]]), 1, 0)
  }

# Filter missing indicators
m.mi <- m.data_sub_noTarget %>% 
...
m.data_sub_noTarget <- m.data_1h_no_dup_Ref1 %>% select(all_of(boruta_vars))
m.data_sub_noTarget_mi <- cbind(m.data_sub_noTarget, m.mi_good)
## 23624 obs, 12 vars
```
...
m.train_sub_noTarget_mi <- m.data_sub_noTarget_mi[m.train_ind, ]
m.test_sub_noTarget_mi <- m.data_sub_noTarget_mi[-m.train_ind, ]

library(mice)
m.train_sub_noTarget_imputed <- mice(data=m.train_sub_noTarget_mi, m=5, method="cart", maxit=10, where = is.na(m.train_sub_noTarget_mi))

# picking one of the inerations to be work on
m.train_complete <- complete(m.train_sub_noTarget_imputed,5)

# check if still NAs after the imputation
sum(is.na(m.train_complete)) ## 0

write.csv(x=m.train_complete, file="./data/clean/train_sub_imputed_noTarget.csv")
```

## Logistic regression model
```{r}
# train_complete_withTarget <- merge(y_train ,x.train_completeData)
# merge(x = y_train, y = m.train_complete, by = NULL)

train_complete_withTarget <- cbind(m.train_complete, sale = y_train$sale)

Log1 = glm(sale~., data = train_complete_withTarget, family = binomial)
summary(Log1)
