...
## Correlation Matrix
```{r}
x.res <- cor(X_train[sapply(X_train, is.numeric)], use='pairwise')
...
correlationMatrix <- cor(m.data_no_dup_cc[,])
...
# Dataframe a is the one that highly correlated with other vars
a <- m.data_no_dup_cc[, highlyCorrelated]
m.highlyCorrelatedVars <-names(a)

data_no_dup_nohighlyCorrelatedVars <- data_no_dup %>% select(-all_of(m.highlyCorrelatedVars))
