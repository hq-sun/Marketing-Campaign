...
ggplot(data = m.test_withSelectedColumn_melted, aes(x=variable, y=value)) %>% 
  geom_boxplot(aes(fill = selected)) %>% 
  geom_point(aes(y=value, group=selected), position = position_dodge(width=0.75)) %>% 
...
#
p2 <- ggplot(m.test_withSelectedColumn_melted, aes(x=variable, y=value, fill=selected)) + 
    geom_boxplot() +
    facet_wrap(~variable, scale="free")

p2

ggplot(data = m.test_withSelectedColumn_melted, aes(x=variable, y=value)) + geom_boxplot(aes(fill=selected))

# m.test$selected <- ifelse(rownames(m.test, m.selected.index), 1, 0)

# d$status <- ifelse(endsWith(d$dataset_id, "H"), "healthy", "disease")

# boxplot(m.selected_data)
# boxplot(m.unselected_data)
# 
# barchart(as.kcca(k5, data = df))
# barchart(sale ~ ., m.unselected_data)
# 
# ggplot(stack(m.test_withSelectedColumn), aes(x = factor(ind, levels = names(selected)), y = values)) + geom_boxplot()
# 
# boxplot(selected)

m.variety <- m.variety[-selected]
p2 <- ggplot(m.test_withSelectedColumn, aes(x=m.variety, fill=selected)) + 
    geom_boxplot() +
    facet_wrap(~m.variety, scale="free")

plot()

m.variety <- names(m.test_withSelectedColumn)
